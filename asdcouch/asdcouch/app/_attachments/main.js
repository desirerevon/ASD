...
$('#couch').on(function(){
	...
	$.ajax({
		...
		"dataType": "json",
		"success": function(data){
		 $.each(date.rows, function(index,note){
		 	console.log(value);
		 	var friend = note.doc.friend;
		 	var important = note.doc.important;
		 	var favorite = note.doc.favorite;
		 	var reminder = note.doc.reminder;
		 	var date = note.doc.date;
		 	var notes = note.doc.notes;
		 	$('#notelist').append
		 		$('<li>').append
		 			$('a').attr("href", "#")
		 				.text(title)
